...
def test_offline(verbosity=1):
    ...
    for node in oliverj_utils.timing.TIMING.tree.nodes.values():
        print(f"{node.func_name}: {list(set(node.parents))} | {numpy.average(node.time)}")
